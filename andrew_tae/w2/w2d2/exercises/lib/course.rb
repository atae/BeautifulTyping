...
class Course
	...
	def initialize(course_name, department, credits, days=[], time_block=0)
		@name = course_name
	...
	def add_student(student_name)
		unless @students.include?(student_name)
			student_name.enroll(self)
		end
	end

	def conflicts_with?(course2_name)
		#byebug
		@days.each do |day|
			if course2_name.days.include?(day)
				if course2_name.time_block == @time_block
					return true
				end
			end
		end
